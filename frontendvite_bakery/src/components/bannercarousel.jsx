...
import Home_Service from '../services/home_service';
...
const Bannercarousel = () => {
  ...
  useEffect(() => {
    const fetchBanner = async () => {
      try {
        const data = await Home_Service.getBanner();
        console.log("Fetched banners:", data); // Add this
        setBanners(data);
      } catch (error) {
          ...
          <SwiperSlide key={banner.id}>
            <div className="slide-container w-full h-full flex items-center justify-center">
              ...
              />
                ...
                <h1 className="text-6xl font-bold mb-4" style={{ color: banner.title_color || '#fff' }}>
                    {banner.title}
                </h1>
