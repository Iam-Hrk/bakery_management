import React from 'react';

// const products = [
//   {
//     id: 1,
//     name: "Chocolate Cake",
//     description: "Rich and creamy chocolate delight.",
//     price: "$12.99",
//     image: "https://via.placeholder.com/300x200?text=Chocolate+Cake"
//   },
//   {
//     id: 2,
//     name: "Strawberry Tart",
//     description: "Sweet and tangy fresh fruit tart.",
//     price: "$9.99",
//     image: "https://via.placeholder.com/300x200?text=Strawberry+Tart"
//   },
//   {
//     id: 3,
//     name: "Croissant",
//     description: "Flaky and buttery croissant.",
//     price: "$3.49",
//     image: "https://via.placeholder.com/300x200?text=Croissant"
//   },
//   {
//     id: 4,
//     name: "Vanilla Cupcake",
//     description: "Soft vanilla cupcake with creamy frosting.",
//     price: "$4.99",
//     image: "https://via.placeholder.com/300x200?text=Vanilla+Cupcake"
//   }
// ];

const Products = ({title, products}) => {
  return (
    <div className="bg-gray-100 py-10 px-4">
      <div className="max-w-7xl mx-auto">
        <h2 className="text-3xl font-bold text-center mb-10 text-gray-800">{title}</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
          {products.map(product => (
            <div key={product.id} className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
              <img src={product.imageUrl} alt={product.name} className="w-full h-48 object-cover" />
              <div className="p-4">
                <h3 className="text-xl font-semibold text-gray-800">{product.name}</h3>
                <p className="text-gray-600 text-sm mt-1">{product.offer}</p>
                <div className="mt-3 text-yellow-600 font-bold">{product.price}</div>
                <div className='flex justify-between items-center'>
                <button className="mt-4 cursor-pointer bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-2 rounded-full text-sm transition duration-300">
                  Add to Cart
                </button>
                <i class="bi bi-heart mt-3 cursor-pointer hover:text-red-600"></i>
                </div>
              </div>
            </div>
          ))}
        </div>
      </div>
    </div>
  );
};

export default Products;
